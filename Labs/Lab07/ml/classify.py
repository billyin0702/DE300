...
class ItemSquare(Transformer):
    ...
    def _transform(self, df):
        for cols, out_col in zip(self.__inputCols, self.__outputCols):
            df = df.withColumn(out_col, col(cols) * col(cols))
        return df
...
def pipeline(data: DataFrame):
    ...
    # Create a square of the numeric features
    square_columns = [f"{col}_square" for col in imputed_columns_numeric]
    square_transformers = ItemSquare(inputCols=imputed_columns_numeric, outputCols=square_columns)
    ...
    # Create the pipeline
    pipeline = Pipeline(stages=[indexer, *imputers_string, imputer_numeric, pairwise_product, square_transformers, assembler, classifier])
    
    # Set up the parameter grid for maximum tree depth
    paramGrid = ParamGridBuilder() \
        .addGrid(classifier.maxDepth, [2, 4, 6, 8, 10]) \
        .addGrid(classifier.numTrees, [10, 100, 1000]) \
        .build()
