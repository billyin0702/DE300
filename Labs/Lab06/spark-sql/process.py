...
def main():
    ...
    # Describe rows with age between 30 and 50
    select_age_rows(data)
    data.show(5)

    # Save CSV
    # NOTE: This function below doesn't actually store the csv, but creates a folder with multiple csv files
    data.write.format('csv').option('header', 'true').mode('overwrite').save('saved_data')
    spark.stop()
